...
/*
   ...
   aOrder.condition(Fact(have_food), Equals, pfalse); // Required: hungry() -> true
   aOrder.effect(Fact(money), Set, pfalse);       // Effect: money() -> false
   aOrder.effect(Fact(have_food), Set, ptrue);      // Effect: hungry() -> false
   //aOrder.effect(Fact(food), Set, 10);
*/



   Action aEat("Eat");
   //aEat.condition(Fact(have_food), Equals, ptrue);      // Required: at() -> loc2
   aEat.condition(Fact(have_food), Equals, ptrue);
   aEat.effect(Fact(hungry), Set, pfalse);
